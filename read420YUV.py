...
def yuv2bgr(filename, height, width, startfrm):
    ...
    numfrm = ps // framesize_420  # 计算输出帧数
    print(numfrm)
    fp.seek(framesize_420 * startfrm, 0)
    if not os.path.exists("data/silent/silent-420-QP42"):
        os.makedirs("data/silent/silent-420-QP42")
    for i in range(numfrm - startfrm):
        ...
        scipy.misc.imsave('data/silent/silent-420-QP42/'+str(i)+'.png', bgr_img)
    fp.close()
    return None

if __name__ == '__main__':
    _ = yuv2bgr(filename='C:/Users/Administrator/Desktop/HM-16.0-1/HM-16.0/bin/rec.yuv', height=288, width=352, startfrm=0)