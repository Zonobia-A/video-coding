...
def imcrop(Yt, height, width, image_size, frame):
    ...
    sum = col * raw
    image_sub = np.zeros(shape=(col * raw, image_size, image_size, 3), dtype='uint8')
    for i in range(raw):
        for j in range(col):
            # print('para: ', j*image_size, i*image_size)
            image_sub[i * col + j] = Yt[i*image_size: (i+1)*image_size, j*image_size: (j+1)*image_size, :]
            scipy.misc.imsave('../对比实验/mother-daughter_cif_256x256/420_all/' + str(frame*sum+i * col + j)+'.jpg', image_sub[i * col + j])
...
PATH = "../对比实验/mother-daughter_cif_256x256/420/"
input_paths = glob.glob(os.path.join(PATH, '*.png'))
input_paths = sorted(input_paths, key=lambda path: int(get_name(path)))
length = len(input_paths)
if not os.path.exists("../对比实验/mother-daughter_cif_256x256/420_all"):
    os.makedirs("../对比实验/mother-daughter_cif_256x256/420_all")
